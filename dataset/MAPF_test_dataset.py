...
class test_MAPFDataset(Dataset):
    def __init__(self, data_path, agent_dim):
        ...
        self.test_action_data = self.action_data
        #self.save("data")
    ...
    def save(self, save_path):
        file_name = "test_dataset.npz"
        file_path = os.path.join(save_path, file_name)
        np.savez(file_path, test_data=self.test_data.cpu().numpy(), action_data=self.action_data.cpu().numpy())
    ...
    def __getitem__(self, idx):
        """
        Args:
            idx (_type_): sample index

        Returns:
            _type_: _description_
        """
        test_data = self.test_data[idx]  # shape: (# of time_step, n, m, feature_dim)
        test_action_info = self.test_action_data[idx] # shape: (# of time_step, n, m, 5)
        test_mask = test_action_info.any(-1)
        test_action_info_result = torch.zeros_like(test_mask, dtype=torch.long)
        test_action_info_result[test_mask] = test_action_info[test_mask].argmax(-1) + 1
        ret_data = {"feature": test_data.float(), "action": test_action_info_result, "mask": test_mask}
        return ret_data
