...
from dataset.MAPF_test_dataset import test_MAPFDataset
...
if __name__ == "__main__":
    ...
    test_data = test_MAPFDataset(args.dataset_path, args.feature_dim)
    print(test_data[0])
    #rain_data = MAPFDataset(args.dataset_path, args.feature_dim)
    # train_loader = DataLoader(train_data, shuffle=False,
    #                           batch_size=args.batch_size, 
    #                           num_workers=1)
    
    # Save train_data and action_data to a .txt file
    # with open("train_data.txt", "w") as f:
    #     for i in range(len(train_data)):
    #         # Convert each training data sample to a string
    #         train_data_sample = train_data[i]["feature"]
    #         action_data_sample = train_data[i]["action"]
    #         # Write the training data and action data to the file
    #         f.write(f"Train data sample {i}:\n")
    #         for row in train_data_sample:
    #             f.write(" ".join(map(str, row)) + "\n")
                
    #         f.write(f"Action data sample {i}:\n")
    #         for row in action_data_sample:
    #             f.write(" ".join(map(str, row)) + "\n")
